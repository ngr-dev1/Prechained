// backfill-b.js — Parallel backfill worker B.
// Same logic as backfill.js but queries rows with id > offset stored in DB.
// Runs simultaneously with backfill.js and backfill-c.js for 3x throughput.
...
// Worker B processes rows in DESCENDING order (newest first)
// Worker A processes ascending (oldest first)
// Together they meet in the middle

async function fetchNpmManifest(pkgName, version) {
  const res = await fetch(`https://registry.npmjs.org/${encodeURIComponent(pkgName)}`, { headers: { Accept: "application/json" } });
  ...
  return {
    ...
    scripts: { install: vd.scripts?.install || null, preinstall: vd.scripts?.preinstall || null, postinstall: vd.scripts?.postinstall || null, prepare: vd.scripts?.prepare || null, prepublish: vd.scripts?.prepublish || null, prepublishOnly: vd.scripts?.prepublishOnly || null },
    maintainers: (vd.maintainers || data.maintainers || []).map(m => ({ name: m.name, email: m.email || null })),
    author: vd.author || data.author || null, publishedAt: data.time?.[version] || null,
...
async function fetchPypiManifest(pkgName, version) {
  ...
  return {
    ...
    requires_python: data.info?.requires_python || null, requires_dist: data.info?.requires_dist || [],
    keywords: data.info?.keywords || null, classifiers: data.info?.classifiers || [],
    project_urls: data.info?.project_urls || {}, yanked: data.info?.yanked || false,
...
async function fetchPackagistManifest(pkgName, version) {
  ...
  return {
    ...
    publishedAt: vData.time || null, github_stars: pkg_data.github_stars || null,
    github_watchers: pkg_data.github_watchers || null, github_forks: pkg_data.github_forks || null,
...
async function fetchNugetManifest(pkgName, version) {
  ...
  for (const page of regData.items || []) {
    ...
    if (!page.items && page["@id"]) {
      ...
      if (pg.ok) { const pgData = await pg.json(); for (const item of pgData.items || []) { if (item.catalogEntry?.version?.toLowerCase() === version.toLowerCase()) { entry = item.catalogEntry; break; } } }
    }
    if (entry) break;
  }
  if (!entry) throw new Error(`version not found`);
  const nugetDeps = (entry.dependencyGroups || []).flatMap(g => (g.dependencies || []).map(d => ({ id: d.id, range: d.range, targetFramework: g.targetFramework || null })));
  return {
...
async function fetchGithubManifest(pkgName, version) {
  ...
  return {
    ...
    default_branch: repo.default_branch, publishedAt: release?.published_at || null,
...
export default async function handler(req, context) {
  ...
  console.log("[backfill-b] start", new Date().toISOString());

  let archiveDone = 0, registryDone = 0, failed = 0;

  // Worker B: descending order — works from newest rows backward
  ...
  for (const row of archiveRows || []) {
    ...
    try {
      ...
      console.warn(`[backfill-b] archive ${pkg.name}@${row.version}: ${e.message}`);
      failed++;
    }
  }
    ...
    if (regIds.length) {
      ...
      for (const row of registryRows || []) {
        ...
        try {
          ...
          try { manifestPath = await storeManifestInGithub(row.ecosystem, pkg.name, row.version, manifest); } catch {}
          const { error } = await supabase.from("snapshots").update({
            sha384_fingerprint: fingerprint,
          ...
          console.warn(`[backfill-b] registry ${pkg?.name}@${row.version}: ${e.message}`);
          failed++;
        }
      }
    }
  ...
  console.log(`[backfill-b] done: archive=${archiveDone} registry=${registryDone} failed=${failed} remaining=${remaining} ${elapsed}ms`);

  return new Response(JSON.stringify({
    ok: true, worker: "b", archiveDone, registryDone,
    totalDone: archiveDone + registryDone, failed, remaining,
